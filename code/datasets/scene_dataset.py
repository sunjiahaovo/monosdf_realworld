...
class SceneDatasetDNIM(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        for i in range(self.n_images):
            if i in remove_list:
                continue
            path = os.path.join(self.instance_dir,str(i)+"_t265_pose.txt")
            # path = os.path.join(self.instance_dir,str(i)+"_optitrack_pose.txt")
        ...
        camera_intrinsic = np.array([[607.89*384/480,0,384/2],[0,607.93*384/480,384/2],[0,0,1]]).astype(np.float64)
        ...
        for i in range(self.n_images):
            ...
            if i < 360:
                theta = i/180*np.pi
                x = -1.5*np.cos(theta)+1.3
                y = 1.3*np.sin(theta)
                z = 0.7
                object_center = np.array([2.4,0,0.8])
            ...
            T_b2o = np.array([[0,0,-1,0],
                              ...
                              [0,0,0,1]])
            pose = np.linalg.inv(T_b2o) @  pose @ T_b2o
            pose_test = np.linalg.inv(T_b2o) @  pose_test @ T_b2o
