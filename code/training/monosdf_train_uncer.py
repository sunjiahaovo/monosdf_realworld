...
class MonoSDFTrainRunner_uncer():
    def __init__(self,**kwargs):
        ...
        if kwargs['render_only']:
                #  mkdir render path
            if os.path.exists(os.path.join("../render_exps/",kwargs['timestamp'],"plots")):
                self.plots_dir = os.path.join("../render_exps/",kwargs['timestamp'],"plots")
            else:
                os.makedirs(os.path.join("../render_exps/",kwargs['timestamp'],"plots"))
                self.plots_dir = os.path.join("../render_exps/",kwargs['timestamp'],"plots")
        ...
        # self.model_uncer = self.copy_model()
        ...
        if kwargs['render_only']:
            ...
            ckpt = os.path.join('../exps2/l435_rgbd_grids_7', kwargs['timestamp'], 'checkpoints/ModelParameters', str(kwargs['checkpoint']) + '.pth')
            print(ckpt)
            ...
            self.model2.load_state_dict(ckpt['model_state_dict'])
        ...
        if kwargs['render_only']:
           ...
           for n in range(0,360,1):
                self.render_test(100,data_type="test_all",index = n, device=device2)
    ...
    def run(self):
        ...
        while(n_index < self.train_dataset.n_images and False):
            ...
            if self.iter_step%10==0 and self.iter_step>0:
        ...
        for epoch in range(self.start_epoch, self.nepochs + 1):
            ...
            if self.GPU_INDEX == 0 and self.do_vis and epoch % self.plot_freq == 0 :
                ...
                self.model2.load_state_dict(self.model.state_dict())
                index_test = np.random.randint(self.train_dataset.n_images,size=1)[0]
                self.render_test(self.iter_step, "train", index_test,device2)
                # threading.Thread(target=self.render_test,args=(self.iter_step,"train", 60,device2)).start()
                # for n in range(0,120,30):
                #     threading.Thread(target=self.render_test,args=(self.iter_step,"test_all", 60,device2)).start()
    ...
    def render_test(self, epoch, data_type = "train", index = 0, device=device):
            ...
            for s in tqdm(split):
                ...
                out = self.model2(s, indices, epoch, device=device)
