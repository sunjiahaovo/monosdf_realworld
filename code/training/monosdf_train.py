    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, self.nepochs + 1):
            ...
            if self.GPU_INDEX == 0 and self.do_vis and epoch % self.plot_freq == 0:
                ...
                split = utils.split_input(model_input, self.total_pixels, n_pixels=1*self.split_n_pixels)
                res = []
                for s in tqdm(split):
            ...
            for data_index, (indices, model_input, ground_truth) in enumerate(self.train_dataloader):
                ...
                if self.GPU_INDEX == 0:
                    print(
                        '{0}_{1} [{2}] ({3}/{4}): loss = {5}, rgb_loss = {6}, eikonal_loss = {7},smooth_loss = {8}, depth_loss = {9}, psnr = {10}, bete={11}, alpha={12}'
