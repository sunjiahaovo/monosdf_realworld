...
def plot(implicit_network, indices, plot_data, path, epoch, img_res, plot_nimgs, resolution, grid_boundary,  level=0,data_type = "train", is_vs = True, device=device, iter_step = 0):
    ...
    if plot_data is not None:
        ...
        if data_type == "train":
            ...
            plot_images(plot_data['rgb_eval'], plot_data['rgb_gt'], path, epoch, plot_nimgs, img_res, indices, is_vs=False, device=device)
        ...
        if data_type == "train":
            ...
            plot_normal_maps(plot_data['normal_map'], plot_data['normal_gt'], path, epoch, plot_nimgs, img_res, indices, is_vs=False, device=device)

        # plot depth maps
        if data_type == "train":
            plot_depth_maps(plot_data['depth_map'], plot_data['depth_gt'], path, epoch, plot_nimgs, img_res, indices, is_vs=is_vs, device=device)
        else:
            plot_depth_maps(plot_data['depth_map'], plot_data['depth_gt'], path, epoch, plot_nimgs, img_res, indices, is_vs=False, device=device)
    ...
    if data_type == "train":
        surface_traces = get_surface_sliding(path=path,
                                            epoch=epoch,
                                            sdf=lambda x: implicit_network(x, iter_step)[:, 0],
